...
function extract_timings(lines, benchmark)
    ...
    if benchmark in ("bbvv", "bbdr", "cpdvv", "bbmmvv")
        timer4 = extract_timer(lines, "reduce_tls_to_gs!")
...
function speedup_plot(df::DataFrame)
    benchmark = df.benchmark[1]
    colors = Makie.wong_colors()
    fig = Figure()
    ax = Axis(fig[1, 1];
              title = TITLE_TL[benchmark],
...
function profiling_plot(df)
    ...
    sce1 = scatterlines!(ax, df.t, last.(df.t_init_body), color = colors[1])
    sce2 = scatterlines!(ax, df.t, last.(df.t_comp_fd), color = colors[2])
    sce3 = scatterlines!(ax, df.t, last.(df.t_update_cache), color = colors[3])
    lg1 = [sce1, sce2, sce3]
    lg1_lbl = ["init. body", "comp. forcedensity", "update cache"]
    if is_timed(df.t_def_cracks)
        sce41 = scatterlines!(ax, df.t, last.(df.t_def_cracks),
                              color = colors[4])
        push!(lg1, sce41)
        push!(lg1_lbl, "def. cracks")
    end
    if is_timed(df.t_comp_cfd)
        sce42 = scatterlines!(ax, df.t, last.(df.t_comp_cfd), color = colors[4])
        push!(lg1, sce42)
        push!(lg1_lbl, "comp. contact")
    end
    sce5 = scatterlines!(ax, df.t, last.(df.t_export_vtk), color = colors[5])
    push!(lg1, sce5)
    push!(lg1_lbl, "vtk export")

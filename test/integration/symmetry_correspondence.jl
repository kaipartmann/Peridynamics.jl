@testitem "symmetry CMaterial VelocityVerlet" begin
    ...
    body = Body(CMaterial(model=NeoHookePenalty()), pos, vol)
...
@testitem "symmetry CMaterial NewtonRaphson" begin
    ...
    dr = NewtonRaphson(steps=5, stepsize=1.0, maxiter=50, tol=1e-3)
    temp_root = mktempdir()
...
@testitem "symmetry CRMaterial VelocityVerlet" begin
    # simulation
    Δx = 0.2
    width = 1
    grid⁺ = Δx/2:Δx:width/2
...
@testitem "symmetry CRMaterial DynamicRelaxation" begin
    # simulation
    Δx = 0.2
    width = 1
    grid⁺ = Δx/2:Δx:width/2

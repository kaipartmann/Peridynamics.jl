...
const SIMNAME = length(ARGS) ≥ 2 ? ARGS[2] : "bbdr"
const EXPORT_VTK = length(ARGS) ≥ 3 ? parse(Bool, ARGS[3]) : true

function mode_I_tension_quasistatic(l, Δx, f0, nt)
    pc = PointCloud(l, l, 0.1l, Δx)
    δ = 3.015Δx
    mat = BondBasedMaterial(horizon=δ, rho=8e-6, E=2.1e5, Gc=2.7)
    a = 0.5l
    ...
    b0::Float64 = f0 / (Δx * 0.1l * l)
    bc_bottom = ForceDensityBC(t -> -b0, set_bottom, 2)
    bc_top = ForceDensityBC(t -> b0, set_top, 2)
    vv = DynamicRelaxation(nt)
...
@time mode_I_tension_quasistatic(1.0, 1/50, 10, 1000) # compilation run
@time mode_I_tension_quasistatic(1.0, 1/150, 10, 1000)

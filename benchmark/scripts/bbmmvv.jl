...
const SIMNAME = length(ARGS) ≥ 2 ? ARGS[2] : "bbmmvv"
const EXPORT_VTK = length(ARGS) ≥ 3 ? parse(Bool, ARGS[3]) : true

function mode_I_tension_dynamic(l, Δx, v0, nt)
    pc = PointCloud(l, l, 0.1l, Δx)
    δ = 3.015Δx
    mat1 = BondBasedMaterial(horizon=δ, rho=2e-6, E=2e4, Gc=0.01)
    mat2 = BondBasedMaterial(horizon=δ, rho=8e-6, E=2.1e5, Gc=2)
    matofpoint = [ifelse(p[1] ≤ 0.25l, 1, 2) for p in eachcol(pc.position)]
    mat = MultiMaterial((mat1, mat2), matofpoint)
    a = 0.5l
...
@time mode_I_tension_dynamic(1.0, 1/150, 10, 2000)

...
function calc_first_piola_kirchhoff!(storage::CRStorage, mat::CRMaterial,
    ...
    Δθ = tr(Δε)
    Δεᵈᵉᵛ = Δε - Δθ / 3 * I
    σ = get_tensor(storage.unrotated_stress, i, dims(storage))
    σₙ₊₁ = σ + 2 * params.G * Δεᵈᵉᵛ + params.K * Δθ * I
    update_tensor!(storage.unrotated_stress, i, σₙ₊₁, dims(storage))
...
function calc_zem_stiffness_tensor!(storage::CRStorage, system::BondSystem, mat::CRMaterial,
    ...
    R = get_tensor(rotation, i, dims(system))
...
function export_field(::Val{:cauchy_stress}, ::CRMaterial, system::BondSystem,
    ...
    for i in each_point_idx(system)
        σ = get_tensor(storage.unrotated_stress, i, dims(system))
        T = rotate_stress(storage, σ, i)
        Peridynamics.update_tensor!(storage.cauchy_stress, i, T, dims(system))
    end

...
function apply_precracks!(chunk::AbstractBodyChunk, body::AbstractBody)
    # without a crack there is nothing to write into the bookkeeping and the damage is
    # zero anyway, so the reduction is skipped and a setup whose damage model carries no
    # bookkeeping stays constructible
    isempty(body.point_sets_precracks) && return nothing

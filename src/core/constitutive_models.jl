...
Return the constitutive model of a material. Every family that stores a constitutive model
...
`nothing`, the answer for a material family that has no constitutive model, e.g. the
...
See also [`first_piola_kirchhoff`](@ref), [`constitutive_state`](@ref).
"""
...
An `InterfaceError` that asks for a method of the model.

# Example

A model that only needs the deformation gradient defines the four-argument form, which this
one is bridged to.
...
A model that integrates a history declares its state with [`@cm_storage`](@ref) and defines
this form, reading and advancing the state through [`constitutive_state`](@ref).
...
A history-dependent model runs only on a material whose storage declares
`cm_state::ConstitutiveState`, which today are [`CMaterial`](@ref), [`RKCMaterial`](@ref)
and [`BACMaterial`](@ref). [`CRMaterial`](@ref) and [`RKCRMaterial`](@ref) do not declare
that field, so a history-dependent model cannot be used with them.

See also [`strain_energy_density`](@ref), [`@cm_storage`](@ref),
[`constitutive_state`](@ref), [`is_history_dependent`](@ref).
"""
...
"""
    ...
    constitutive_storage_type(model, ::Type{FT}, ::Val{N})

$(extension_api_note())

Return the type of the state of a constitutive model, instantiated for the float type `FT`
and the number of spatial dimensions `N` of the simulation. This method is generated by
[`@cm_storage`](@ref), so a model declared with that macro needs nothing here.

# Default

`Nothing`, the answer for a model without state.

See also [`@cm_storage`](@ref), [`constitutive_state`](@ref), [`storage_type`](@ref).
"""
...
constitutive_storage_type(model, ::Type, ::Val) = constitutive_storage_type(model)
...
declared with `cm_state::ConstitutiveState`. This is how
[`first_piola_kirchhoff`](@ref) reaches the state of a history-dependent model.

# Default

`nothing`, the answer for a storage that declares no such field.

See also [`@cm_storage`](@ref), [`first_piola_kirchhoff`](@ref), [`damage_state`](@ref).
"""
...
strain of a plasticity model. A history-dependent model may only be used with a time solver
that evaluates the force density once per time step, and only with a material whose storage
carries the state. Both are checked once when a [`Job`](@ref) is created.

# Default

Whether the model declares a state with [`@cm_storage`](@ref), so a model that keeps its
history in the storage of the material has to declare it itself.
...
See also [`supports_history_dependence`](@ref), [`@cm_storage`](@ref),
...
not always index the same quantity, because the state of the model could then not be indexed
at all.

# Default

`true` for both a time solver and a material.

See also [`is_history_dependent`](@ref), [`first_piola_kirchhoff`](@ref).
"""

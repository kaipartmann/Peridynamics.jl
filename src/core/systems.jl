...
spatial dimensions `N` of the simulation. A system generated by [`@system`](@ref) is
...
A material inherits the method from the material family it subtypes, so only a new system
needs one, and then it is a single line over `host_system_type`:
...
the wrong material family fails at setup with a readable message instead of somewhere inside
a kernel.

A system restricts its materials by defining the pair of methods that
[`BondSystem`](@ref) does, one that accepts its material family and one that rejects
everything else. A system without a restriction needs no method, the fallback accepts every
material.
"""
...
Return the largest number of chunks a body of the material `mat` may be decomposed into.
The default is `typemax(Int)`, so a material places no limit and the number of chunks
follows the number of threads or MPI ranks.

A system that cannot be split defines the method and returns `1`, e.g. a system that
transforms the whole body at once and therefore needs every point of it in one chunk. A
threaded run silently clamps the number of chunks to this value, an MPI run with more ranks
than the material allows is an error, because the ranks are given from the outside and
...
of its system, so the two can never disagree.

Every system of this package is 3D, so this always returns `3` today.
...
`AbstractBondSystemMaterial`. With [`NoCorrection`](@ref) the factor is `1`, so a material can
always write the multiplication and pays nothing when no correction is used. A system without
a `correction` field, e.g. a [`BondAssociatedSystem`](@ref), has no surface correction and the
factor is `1` as well, resolved at compile time with `hasfield`, which is what lets a bond
based kernel run on every bond bearing system.
...
CPU, for `N` spatial dimensions and the float type `FT`. Every array field of such a system
is a type parameter, so that a whole chunk can be moved to another array backend with
`Adapt.adapt`, and this is the instantiation a simulation that stays on the CPU uses. It is
what [`system_type`](@ref) returns, so that the body chunks of a data handler are concrete.

The `user_params` are the type parameters the system declares itself, in the order it
declares them, each of them already a host type, see [`host_type`](@ref).
"""
...
system is allocated by exactly the same [`alloc_field`](@ref) the storage side uses:
...
It answers the questions the shape layer asks, [`get_n_dim`](@ref), [`float_type`](@ref),
the point counts through the chunk handler and [`get_n_bonds`](@ref), and nothing else.
"""
...
Define a system, the discretization of a body chunk. This is [`@storage`](@ref) for a system:
the body accepts the same field declarations, so a field of a system is allocated, sized and
moved to another array backend exactly like a storage field. Unlike a storage, a system has
no reusable field blocks: `@storage_fields` and `@inherit` are for a storage, and writing
`@inherit` inside `@system` is an error. A system lists every one of its fields directly.
